...
+ (instancetype)share {
    ...
    dispatch_once(&onceToken, ^{
        ...
        manager.syncQueue = dispatch_queue_create("YLCFNotificationManager", DISPATCH_QUEUE_CONCURRENT);
    });
    return manager;
}
...
static void YLCFNotificationCallback(CFNotificationCenterRef center, void *observer, CFStringRef name, const void *object, CFDictionaryRef userInfo) {
    ...
    dispatch_async([YLCFNotificationManager share].syncQueue, ^{
...
- (void)addObserver:(id)observer selector:(SEL)selector name:(NSNotificationName)name {
    if(name.length > 0) {
        dispatch_barrier_async([YLCFNotificationManager share].syncQueue, ^{
